...
void Player::FakeDisplayID()
{
    ...
    if (!NativeTeam())
    {
        ...
        if (!info)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Player %u has incorrect race/class pair. Can't init display ids.", GetGUIDLow());
            return;
        }
        ...
        case GENDER_MALE:
            ...
            break;
        default:
            sLog.Out(LOG_BASIC, LOG_LVL_ERROR, "Invalid gender %u for player", gender);
            return;
        }
    }
}
