        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17 fishing bobber
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    // auto fish: zone fishing skill will be higher 
                    if (zone_skill < 0)
                    {
                        zone_skill = 0;
                    }
                    ...
                    else if (sWorld.getConfig(CONFIG_BOOL_SKILL_FAIL_LOOT_FISHING))
                    {
                        ...
                        // auto fish: fishing fail hint 
                        std::ostringstream notificationStream;
                        notificationStream << "需要釣魚技能高於 " << zone_skill;
