...
void CombatBotBaseAI::PopulateSpellData()
{
    ...
    for (const auto& spell : me->GetSpellMap())
    {
		...
		// Check Hidden Spells and Passive talents 檢查有無具備所需隱藏被動法術
		if (pSpellEntry->HasAttribute(SPELL_ATTR_PASSIVE))
		{
			switch (me->GetClass())
			{
				case CLASS_DRUID:
				{
					if (pSpellEntry->SpellName[0].find("Furor") != std::string::npos) //激怒
					{
				...
				case CLASS_MAGE:
				{
					if (pSpellEntry->SpellName[0].find("Improved Arcane Explosion") != std::string::npos) //強化魔爆術
					{
        ...
        switch (me->GetClass())
        {
            case CLASS_PALADIN:
            {
                ...
                else if (pSpellEntry->SpellName[0].find("Seal of Wisdom") != std::string::npos) //智慧聖印
                {
                ...
                else if (pSpellEntry->SpellName[0].find("Seal of the Crusader") != std::string::npos) //十字軍聖印
                {
            ...
            case CLASS_SHAMAN:
            {
                ...
                {
                    if (!pWindfuryWeapon ||
                        pWindfuryWeapon->Rank < pSpellEntry->Rank)
            ...
            case CLASS_HUNTER:
            {
				...
				else if (pSpellEntry->SpellName[0].find("Frost Trap") != std::string::npos) //冰霜陷阱
				{
					if (!m_spells.hunter.pFrostTrap ||
						m_spells.hunter.pFrostTrap->Rank < pSpellEntry->Rank)
				...
				else if (pSpellEntry->SpellName[0].find("Revive Pet") != std::string::npos) //復活寵物
				{
					if (!m_spells.hunter.pRevivePet ||
						m_spells.hunter.pRevivePet->Rank < pSpellEntry->Rank)
                ...
                else if (pSpellEntry->SpellName[0].find("Trueshot Aura") != std::string::npos) //強擊光環
                {
            ...
            case CLASS_MAGE:
            {
				...
				else if (pSpellEntry->SpellName[0].find("Portal: Ironforge") != std::string::npos) //傳送門： 鐵爐堡
				{
					if (!m_spells.mage.pPortalIronforge ||
						m_spells.mage.pPortalIronforge->Rank < pSpellEntry->Rank)
            ...
            case CLASS_PRIEST:
            {
                ...
                else if (pSpellEntry->SpellName[0].find("Prayer of Healing") != std::string::npos) //治療禱言
                {
            ...
            case CLASS_WARLOCK:
            {
                ...
                if (pSpellEntry->SpellName[0].find("Demon Skin") != std::string::npos) //惡魔皮膚
                {
                ...
                else if (pSpellEntry->SpellName[0].find("Dark Pact") != std::string::npos) //黑暗契約
                {
				...
				else if (pSpellEntry->SpellName[0].find("Unending Breath") != std::string::npos)
				{
					if (!m_spells.warlock.pUnendingBreath ||
						m_spells.warlock.pUnendingBreath->Rank < pSpellEntry->Rank)
            ...
            case CLASS_WARRIOR:
            {
				...
				else if (pSpellEntry->SpellName[0]._Equal("Slam")) //猛擊
				{
            ...
            case CLASS_ROGUE:
            {
                ...
                else if (pSpellEntry->SpellName[0].find("Sap") != std::string::npos) //悶棍
                {
        ...
        for (GroupReference* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            if (Player* pMember = itr->getSource())
            {
				...
				*/
                ...
                // Check if healing is being cast by someone else 檢查是否已被其他人治療中
